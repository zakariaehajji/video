"""One-shot runner for V22_pacing_breathe_v2 experiment.

Hypothesis: V19 breathe REJECTED (8.57 vs V15 8.73) because it homogenized
durations (dur_var≈0.009 → pacing 5.5) via too many mid-peak extends + raised
story peak floors. A milder V2 breathe — max 3 short mid holds (+0.16s), protect
top-2 climaxes, abort if variance would fall below 0.055, polish after V21
face/reaction grammar, no story floor raise — should ease busy mid-peak without
flattening pacing/music sync.
...
reaction cutaways v2. Does NOT enable V19 PACE_BREATHE (aggressive).
"""
...
os.environ["WEDDING_V3_PACE_BREATHE_V2"] = "1"
...
V22_DIR = ROOT / "Output" / "autolab" / "V22"
V21_BEST = 8.84
V20_BEST = 8.82
V15_BEST = 8.73
V10_BEST = 8.69
...
def _assert_wedding_pool() -> None:
    ...
    if len(videos) < 8 or any("whatsapp" in s for s in stems) or any(
        ...
        raise RuntimeError(
            ...
            "Restore from wedding_* caches before running V22."
        )
...
def _pace_breathe_stats(run_dir: Path) -> dict:
    ...
    breathes = [
        p
        for p in picks
        if "pace-breathe-v2" in (p.get("reasons") or [])
        or "pace-breathe" in (p.get("reasons") or [])
    ]
    ...
    return {
        ...
        "breathe_shot_ids": [p.get("shot_id") for p in breathes],
    }
...
def main() -> int:
    ...
    if hasattr(story_mod, "PACE_BREATHE_V2"):
        story_mod.PACE_BREATHE_V2 = True
    ...
    ranking_mod.PACE_BREATHE_V2 = True
    ...
    print("=== V22_pacing_breathe_v2 render ===", flush=True)
    ...
    if not run_result.get("ok"):
        ...
        eval_data = {
            "experiment": "V22_pacing_breathe_v2",
            "status": "failed",
            "error": err,
        ...
        (V22_DIR / "EVAL.json").write_text(json.dumps(eval_data, indent=2), encoding="utf-8")
        append_experiment_log(f"V22_pacing_breathe_v2 FAILED: {err}")
        return 1

    run_dir = Path(run_result["run_dir"])
    for mp4 in run_dir.glob("*.mp4"):
        dest = V22_DIR / mp4.name
    ...
    if verdict == "KEEP" and best_src and Path(best_src).exists():
        dest = ROOT / "Output" / "autolab" / "BEST_v22.mp4"
        ...
        try:
            ...
            eid = lab_db.insert_experiment(
                version="V22_pacing_breathe_v2",
                hypothesis=(
                    "Milder breathe on top of face-protected + reaction grammar "
                    "without V19 variance collapse"
                ),
                configuration={
                    "tag": TAG,
                    "pace_breathe_v2": True,
            ...
            lab_db.promote_version(
                "V22_pacing_breathe_v2",
                score,
                promoted,
                notes=f"pace_breathe_v2 KEEP delta={eval_result.get('delta')}",
    ...
    eval_data = {
        ...
        "status": "completed",
        "hypothesis": (
            "Milder breathe (max 3 × +0.16s, variance guard) on top of "
            "face-protected + reaction grammar"
        ),
        "v21_baseline": V21_BEST,
    ...
    append_experiment_log(
        "\n".join(
            [
                "V22_pacing_breathe_v2 completed.",
                f"run_dir={run_dir}",
                f"pace={pace_stats}",
    ...
    state = load_state()
    state.current_experiment = "V22_pacing_breathe_v2"
    state.current_version = "V22_pacing_breathe_v2"
    state.experiments_run = int(getattr(state, "experiments_run", 0) or 0) + 1
    state.candidates_run += len(leaderboard.get("rendered", []))
    if verdict == "KEEP":
        state.best_version = "V22_pacing_breathe_v2"
        state.best_score = score
        fails = [f for f in (state.failed_experiments or []) if f != "V22_pacing_breathe_v2"]
        state.failed_experiments = fails
        state.last_strategy = "KEEP V22 pacing_breathe_v2; next: kiss_hold_polish"
        state.next_tasks = [
            {
                "title": "V23_kiss_hold_polish",
                "hypothesis": "Hold strongest kiss climax longer without busy mid-peak spray",
                "priority": 1.0,
            },
            {
                "title": "V23_reaction_portrait_bias",
        ...
        if "V22_pacing_breathe_v2" not in fails:
            fails.append("V22_pacing_breathe_v2")
        state.failed_experiments = fails
        state.best_version = "V21_reaction_cutaways_v2"
        state.best_score = V21_BEST
        state.last_strategy = "REJECT V22 pacing_breathe_v2; keep V21; next: kiss_hold_polish"
        state.next_tasks = [
            {
                "title": "V23_kiss_hold_polish",
                "hypothesis": "Hold strongest kiss climax longer without busy mid-peak spray",
    ...
    try:
        ...
        jobs = []
    jobs = [j for j in jobs if j.get("id") != "V22_pacing_breathe_v2"]
    jobs.append(
        {
            "id": "V22_pacing_breathe_v2",
            "script": "autolab/run_v22_pacing_breathe_v2.py",
            "status": "done",
            "priority": 18,
            "hypothesis": (
                "Milder breathe on top of face-protected + reaction grammar"
    ...
    blocker = {
        ...
        "note": f"V22_pacing_breathe_v2 {verdict} {score:.2f} vs V21 {V21_BEST}",

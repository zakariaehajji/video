"""One-shot runner for V25_kiss_ensure_swap experiment.

Hypothesis: V23 kiss-hold REJECT (8.77 vs V21 8.84) because ensure never fired
(peak already had kiss>=0.38) while stronger unused kiss+hug climaxes
(e.g. wedding_40601_005) stayed out of peak. Force-swap the top unused pool
kiss into the late-peak climax window WITHOUT duration holds.
...
reaction cutaways v2. Does NOT enable V23 KISS_HOLD, V11/V14 PEAK_HOLD,
V19/V22 PACE_BREATHE, V24 CONTINUITY_SOFT_V2.
"""
...
os.environ["WEDDING_V3_KISS_ENSURE_SWAP"] = "1"
...
V25_DIR = ROOT / "Output" / "autolab" / "V25"
V21_BEST = 8.84
V20_BEST = 8.82
V15_BEST = 8.73
V10_BEST = 8.69
...
def _assert_wedding_pool() -> None:
    ...
    if len(videos) < 8 or any("whatsapp" in s for s in stems) or any(
        ...
        raise RuntimeError(
            ...
            "Restore from wedding_* caches before running V25."
        )


def _kiss_ensure_stats(run_dir: Path) -> dict:
    ...
    ensures = [p for p in picks if "kiss-ensure-swap" in (p.get("reasons") or [])]
    tops = [p for p in picks if "kiss-ensure-top" in (p.get("reasons") or [])]
    ...
    return {
        ...
        "ensure_kisses": [
            round(kiss_by_id.get(str(p.get("shot_id") or ""), 0.0), 4) for p in ensures
        ],
...
def main() -> int:
    ...
    if not run_result.get("ok"):
        ...
        eval_data = {
            "experiment": "V25_kiss_ensure_swap",
            "status": "failed",
            "error": err,
        ...
        (V25_DIR / "EVAL.json").write_text(json.dumps(eval_data, indent=2), encoding="utf-8")
        append_experiment_log(f"V25_kiss_ensure_swap FAILED: {err}")
        return 1

    run_dir = Path(run_result["run_dir"])
    for mp4 in run_dir.glob("*.mp4"):
        dest = V25_DIR / mp4.name
        if mp4.resolve() != dest.resolve():
            shutil.copy2(mp4, dest)

    kiss_stats = _kiss_ensure_stats(run_dir)
    # Experiment is invalid if ensure never fired — treat as failed mechanism.
    ensure_ok = int(kiss_stats.get("n_kiss_ensures") or 0) >= 1
    ...
    verdict = "KEEP" if eval_result.get("better_than_best") and ensure_ok else "REJECT"
    if not ensure_ok and eval_result.get("better_than_best"):
        print("WARN: score up but n_kiss_ensures=0 — REJECT (mechanism miss)", flush=True)
    best = leaderboard.get("best") or {}
    best_src = best.get("path")
    promoted = None
    if verdict == "KEEP" and best_src and Path(best_src).exists():
        dest = ROOT / "Output" / "autolab" / "BEST_v25.mp4"
        ...
        try:
            ...
            eid = lab_db.insert_experiment(
                version="V25_kiss_ensure_swap",
                hypothesis=(
                    "Force top unused pool kiss into peak climax without V23 hold"
                ),
                configuration={
                    "tag": TAG,
                    "kiss_ensure_swap": True,
                    "kiss_hold": False,
            ...
            lab_db.promote_version(
                "V25_kiss_ensure_swap",
                score,
                promoted,
                notes=f"kiss_ensure_swap KEEP delta={eval_result.get('delta')}",
    ...
    eval_data = {
        ...
        "status": "completed",
        "hypothesis": (
            "Force top unused pool kiss into peak climax when V23 hold-alone "
            "ensure never fired (on top of V21 face+reaction grammar)"
        ),
        "v21_baseline": V21_BEST,
    ...
    append_experiment_log(
        "\n".join(
            [
                "V25_kiss_ensure_swap completed.",
                f"run_dir={run_dir}",
                f"kiss={kiss_stats}",
                f"best_score={score} (V21 baseline {V21_BEST})",
                f"verdict={verdict}",
                f"ensure_ok={ensure_ok}",
    ...
    state = load_state()
    state.current_experiment = "V25_kiss_ensure_swap"
    state.current_version = "V25_kiss_ensure_swap"
    state.experiments_run = int(getattr(state, "experiments_run", 0) or 0) + 1
    state.candidates_run += len(leaderboard.get("rendered", []))
    if verdict == "KEEP":
        state.best_version = "V25_kiss_ensure_swap"
        state.best_score = score
        fails = [f for f in (state.failed_experiments or []) if f != "V25_kiss_ensure_swap"]
        state.failed_experiments = fails
        state.last_strategy = "KEEP V25 kiss_ensure_swap; next: visual_cq_soft_v3"
        state.next_tasks = [
            {
                "title": "V26_visual_cq_soft_v3",
                "hypothesis": "Attack visual_quality~6.3 with CQ nudge under V25 stack",
                "priority": 1.0,
            },
            {
                "title": "V26_reaction_portrait_bias",
        ...
        if "V25_kiss_ensure_swap" not in fails:
            fails.append("V25_kiss_ensure_swap")
        state.failed_experiments = fails
        state.best_version = "V21_reaction_cutaways_v2"
        state.best_score = V21_BEST
        state.last_strategy = "REJECT V25 kiss_ensure_swap; keep V21; next: visual_cq_soft_v3"
        state.next_tasks = [
            {
                "title": "V26_visual_cq_soft_v3",
            ...
            {
                "title": "V26_kiss_climax_reposition",
    ...
    try:
        ...
        jobs = []
    jobs = [j for j in jobs if j.get("id") != "V25_kiss_ensure_swap"]
    jobs.append(
        {
            "id": "V25_kiss_ensure_swap",
            "script": "autolab/run_v25_kiss_ensure_swap.py",
            "status": "done",
            "priority": 21,
            "hypothesis": "Force top unused pool kiss into peak climax without V23 hold",
    ...
    blocker = {
        ...
        "note": f"V25_kiss_ensure_swap {verdict} {score:.2f} vs V21 {V21_BEST} ensure_ok={ensure_ok}",

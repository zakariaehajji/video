"""One-shot runner for V20_peak_payoff_faces experiment.

Hypothesis: Require eye-contact/face (+ emotion floor) on music peaks via hard
ranking penalties and post-allocate swaps to fix weak visual payoff / no-face
craft failures — without ceremony ring-detail regressions (V17).
...
V16 reactions, V17 ceremony, or V19 pace_breathe.
"""
...
V20_DIR = ROOT / "Output" / "autolab" / "V20"
V15_BEST = 8.73
V10_BEST = 8.69
V8_BEST = 8.60
V3 = 8.15
TAG = "autolab_peak_payoff_faces"
...
def _rebuild_wedding_pool() -> dict:
    ...
    faces = [int(s.faces) for s in shots]
    return {
        "n_shots": len(shots),
        ...
        "faces_ge_1": sum(1 for f in faces if f >= 1),
        "face_emo_ge_040": sum(
            1
            for s in shots
            if int(s.faces) >= 1 and float(s.emotion_score) >= 0.40
        ),
    }
...
def _assert_wedding_pool() -> None:
    ...
    if len(videos) < 8 or any("whatsapp" in s for s in stems) or any(
        ...
        raise RuntimeError(
            ...
            "Restore from wedding_* caches before running V20."
        )


def _peak_face_stats(run_dir: Path) -> dict:
    ...
    forced = [p for p in picks if "peak-face-forced" in (p.get("reasons") or [])]
    upgraded = [p for p in picks if "peak-face-upgrade" in (p.get("reasons") or [])]
    eye = [p for p in picks if "peak-eye-contact" in (p.get("reasons") or [])]
    face_counts = []
    emos = []
    no_face = 0
    weak = 0
    for p in peaks:
        s = by_id.get(p.get("shot_id") or "", {})
        f = int(s.get("faces") or 0)
        e = float(p.get("emotion") or s.get("emotion_score") or 0.0)
        face_counts.append(f)
        emos.append(e)
        if f < 1:
            no_face += 1
        if e < 0.40:
    ...
    return {
        "n_picks": len(picks),
        "n_peaks": len(peaks),
        "peak_no_face": no_face,
        "peak_weak_emo_lt_040": weak,
        "peak_mean_emotion": round(sum(emos) / max(1, len(emos)), 4),
        "peak_mean_faces": round(sum(face_counts) / max(1, len(face_counts)), 4),
        "n_forced_swaps": len(forced),
        "n_upgrades": len(upgraded),
        "n_eye_contact_reasons": len(eye),
        "forced_shot_ids": [p.get("shot_id") for p in forced],
        "upgrade_shot_ids": [p.get("shot_id") for p in upgraded],
    }
...
def main() -> int:
    ...
    print("=== V20_peak_payoff_faces render ===", flush=True)
    ...
    if not run_result.get("ok"):
        ...
        eval_data = {
            "experiment": "V20_peak_payoff_faces",
            "status": "failed",
            "error": err,
        ...
        (V20_DIR / "EVAL.json").write_text(json.dumps(eval_data, indent=2), encoding="utf-8")
        append_experiment_log(f"V20_peak_payoff_faces FAILED: {err}")
        return 1

    run_dir = Path(run_result["run_dir"])
    for mp4 in run_dir.glob("*.mp4"):
        dest = V20_DIR / mp4.name
        if mp4.resolve() != dest.resolve():
            shutil.copy2(mp4, dest)

    peak_stats = _peak_face_stats(run_dir)
    ...
    if verdict == "KEEP" and best_src and Path(best_src).exists():
        dest = ROOT / "Output" / "autolab" / "BEST_v20.mp4"
        ...
        try:
            ...
            eid = lab_db.insert_experiment(
                version="V20_peak_payoff_faces",
                hypothesis=(
                    "Require eye-contact/face on music peaks to fix weak visual payoff"
                ),
                configuration={
                    "tag": TAG,
            ...
            lab_db.promote_version(
                "V20_peak_payoff_faces",
                score,
                promoted,
                notes=f"peak_payoff_faces KEEP delta={eval_result.get('delta')}",
    ...
    append_experiment_log(
        "\n".join(
            [
                "V20_peak_payoff_faces completed.",
                f"run_dir={run_dir}",
                f"peak_face={peak_stats}",
    ...
    state = load_state()
    state.current_experiment = "V20_peak_payoff_faces"
    state.current_version = "V20_peak_payoff_faces"
    ...
    if verdict == "KEEP":
        ...
        fails = [f for f in (state.failed_experiments or []) if f != "V20_peak_payoff_faces"]
        state.failed_experiments = fails
        state.last_strategy = "KEEP V20 peak_payoff_faces; next: reaction_cutaways_v2"
        state.next_tasks = [
            {
                "title": "V21_reaction_cutaways_v2",
                "hypothesis": "Force 2–3 true reaction cutaways after intimacy without V16 zero-swap",
                "priority": 1.0,
            },
            {
                "title": "V21_pacing_breathe_v2",
        ...
        if "V20_peak_payoff_faces" not in fails:
            fails.append("V20_peak_payoff_faces")
        state.failed_experiments = fails
        state.best_version = "V15_emotion_model_refresh"
        state.best_score = V15_BEST
        state.last_strategy = "REJECT V20 peak_payoff_faces; keep V15; next: reaction_cutaways_v2"
        state.next_tasks = [
            {
                "title": "V21_reaction_cutaways_v2",
                "hypothesis": "Force 2–3 true reaction cutaways after intimacy without V16 zero-swap",
            ...
            {
                "title": "V21_peak_payoff_faces_v2",
                "hypothesis": "Softer face floor (emo≥0.35) + sharper close-up bias only",
                "priority": 0.75,
            },
        ]
    save_state(state)
    ...
    try:
        ...
        jobs = []
    jobs = [j for j in jobs if j.get("id") != "V20_peak_payoff_faces"]
    jobs.append(
        {
            "id": "V20_peak_payoff_faces",
            "script": "autolab/run_v20_peak_payoff_faces.py",
            "status": "done",
            "priority": 16,
            "hypothesis": (
                "Require eye-contact/face on music peaks to fix weak visual payoff"
    ...
    blocker = {
        ...
        "note": f"V20_peak_payoff_faces {verdict} {score:.2f} vs V15 {V15_BEST}",
